...
export function ServiceCard({ name, icon: Icon, price, description, features, imageId, imageUrl, galleryUrls = [] }: ServiceCardProps) {
  ...
  return (
    <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
      <Card className="flex flex-col h-full bg-card/40 border-white/5 hover:border-primary/30 transition-all duration-500 group overflow-hidden rounded-none shadow-2xl relative">
        <DialogTrigger asChild>
          <div className="relative h-80 overflow-hidden bg-background/50 cursor-pointer">
            ...
            <div className="absolute top-6 right-6 bg-primary/95 backdrop-blur-md text-primary-foreground px-5 py-2 rounded-none text-[10px] font-black uppercase tracking-widest shadow-2xl z-30">
              {price}
            </div>

            <div className="absolute bottom-6 left-8 flex items-center gap-4 z-30">
              <div className="w-12 h-12 rounded-none bg-primary text-primary-foreground flex items-center justify-center shadow-2xl group-hover:rotate-6 transition-transform">
                <Icon size={24} />
              </div>
          ...
          <div className="space-y-4">
            {features && features.slice(0, 3).map((feature, i) => (
              <div key={i} className="flex items-start gap-3 text-[10px] font-bold uppercase tracking-wider opacity-60">
                <div className="w-5 h-5 rounded-none bg-primary/20 flex items-center justify-center text-primary shrink-0 mt-0.5">
        ...
        <CardFooter className="p-8 pt-6 flex flex-col gap-4">
          <DialogTrigger asChild>
            <Button variant="outline" className="w-full rounded-none h-14 font-bold uppercase tracking-widest text-[10px] border-primary/20 bg-primary/5 hover:bg-primary/10 text-primary">
              <Info size={16} className="mr-2" /> Detail Selengkapnya
            </Button>
          </DialogTrigger>

          <Button asChild className="w-full rounded-none h-16 font-black uppercase tracking-widest text-[11px] shadow-2xl shadow-primary/30 hover:scale-[1.03] transition-all" variant="default">
      ...
      <DialogContent className="max-w-5xl w-[95vw] h-[90vh] rounded-none border-white/10 bg-card p-0 overflow-hidden shadow-2xl flex flex-col z-[60]">
        ...
        <ScrollArea className="flex-1">
          <div className="flex flex-col">
            <div className="relative h-[65vh] sm:h-[80vh] bg-black/50 flex items-center justify-center border-b border-white/5 overflow-hidden">
               ...
               <div className="absolute top-6 left-6 z-30 bg-primary px-5 py-2 rounded-none text-primary-foreground text-[10px] font-black uppercase tracking-widest shadow-2xl">
                 {price}
               </div>
            </div>
            ...
            <div className="p-8 sm:p-16 space-y-12">
              <DialogHeader className="space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center gap-6">
                  <div className="w-20 h-20 rounded-none bg-primary/10 text-primary flex items-center justify-center shrink-0 shadow-inner">
                    <Icon size={48} />
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                       ...
                       <span className="h-1.5 w-1.5 rounded-none bg-primary/30" />
                ...
                <div className="lg:col-span-5 space-y-4">
                  ...
                  <div className="grid gap-3">
                    {features && features.map((feature, i) => (
                      <div key={i} className="flex items-center gap-4 p-5 rounded-none bg-secondary/30 border border-white/5 shadow-sm">
                        <div className="w-6 h-6 rounded-none bg-primary text-primary-foreground flex items-center justify-center shrink-0 shadow-lg">
              ...
              <div className="pt-12 border-t border-white/5 flex flex-col sm:flex-row gap-5">
                <Button onClick={handleOrderClick} size="lg" className="flex-1 rounded-none h-16 sm:h-20 font-black uppercase tracking-widest text-sm shadow-2xl shadow-primary/30 transition-transform active:scale-95">
                  <ShoppingCart className="mr-3" size={24} /> Pesan Sekarang via WA
                </Button>
                <DialogClose asChild>
                  <Button variant="ghost" className="rounded-none h-16 sm:h-20 px-12 font-bold uppercase text-[11px] tracking-widest hover:bg-white/5 border border-white/5">
                    Kembali
                  </Button>
                </DialogClose>
              </div>
