...
export function ServiceCard({ name, icon: Icon, price, description, features, imageId, imageUrl, galleryUrls = [] }: ServiceCardProps) {
  ...
  return (
    <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
      <Card className="flex flex-col h-full bg-card/40 border-white/5 hover:border-primary/30 transition-all duration-500 group overflow-hidden rounded-lg shadow-2xl relative">
          ...
          <div className="space-y-4">
            {features && features.slice(0, 3).map((feature, i) => (
              <div key={i} className="flex items-start gap-3 text-[10px] font-bold uppercase tracking-wider opacity-60">
                <div className="w-5 h-5 rounded-md bg-primary/20 flex items-center justify-center text-primary shrink-0 mt-0.5">
      ...
      <DialogContent className="max-w-5xl w-[95vw] h-[90vh] rounded-lg border-white/10 bg-card p-0 overflow-hidden shadow-2xl flex flex-col z-[60]">
            ...
            <div className="relative h-[65vh] sm:h-[80vh] bg-black/50 flex items-center justify-center border-b border-white/5 overflow-hidden">
               {allImages.length > 0 && (
                  <Carousel className="w-full h-full flex items-center justify-center">
                    ...
                    {allImages.length > 1 && (
                      <>
                        <CarouselPrevious className="left-6 bg-black/40 border-none text-white hidden sm:flex rounded-full" />
                        <CarouselNext className="right-6 bg-black/40 border-none text-white hidden sm:flex rounded-full" />
                      </>
                    )}
                  </Carousel>
               )}
               <div className="absolute top-6 left-6 z-30 bg-primary px-5 py-2 rounded-lg bg-primary/95 text-primary-foreground text-[10px] font-black uppercase tracking-widest shadow-2xl">
                 {price}
               </div>
            </div>
                  ...
                  <div className="grid gap-3">
                    {features && features.map((feature, i) => (
                      <div key={i} className="flex items-center gap-4 p-5 rounded-lg bg-secondary/30 border border-white/5 shadow-sm">
                        <div className="w-6 h-6 rounded-md bg-primary text-primary-foreground flex items-center justify-center shrink-0 shadow-lg">
